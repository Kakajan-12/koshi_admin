...
const Allergens = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/allergens`, {
                    headers: {
    ...
    const handleDelete = async () => {
        ...
        try {
            const token = localStorage.getItem('auth_token');
            await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/allergens/${selectedDataId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });

            setData(prev => prev.filter(p => p.id !== selectedDataId));
            setIsDeleting(false);
            setShowModal(false);
            ...
            <div className="flex-1 p-10 ml-62">
                ...
                <div className="mt-8">
                    <div className="w-full flex justify-between">
                        ...
                        <Link href="/admin/allergens/add-allergens"
                    ...
                    <table className="min-w-full bg-white border border-gray-200 rounded-lg">
                        ...
                        {data.length === 0 ? (
                            ...
                            data.map((data) => (
                                <tr key={data.id} className="divide-x-1 divide-gray-200">
                                    ...
                                    <td className="py-4 px-4 h-full">
                                        <div className="flex justify-center items-center h-full">
                                            <Link
                                                href={`/admin/allergens/edit-allergens/${data.id}`}
